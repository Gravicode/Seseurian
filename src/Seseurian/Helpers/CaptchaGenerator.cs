...
namespace Seseurian.Helpers
{
    ...
    {
        public static byte[] GetCaptcha(string sCaptchaText, int Width = 190, int Height = 80, bool useSystemDrawing = false)
        {
            if (useSystemDrawing)
            {
                int iHeight = Height;
 ...
 HatchStyle.WideDownwardDiagonal, HatchStyle.WideUpwardDiagonal, HatchStyle.ZigZag
            ...
            {
                var gen = new SkiaCaptchaGenerator(paintColor: SKColors.WhiteSmoke, backgroundColor: SKColors.DarkSlateBlue, noisePointColor: SKColors.Yellow, imageWidth: Width, imageHeight: Height, fontName: null, fontSize:20,enableDistortion:true);
                var bytes = gen.GenerateImageAsByteArray(sCaptchaText);
                return bytes;
            }
        }
    }
